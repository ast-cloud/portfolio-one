...
const ProjectCard = ({index, name, description, tags, image, source_code_link, deployment_link}:{index: number, name: string, description: string, tags: any, image: any, source_code_link: string, deployment_link: string})=>{
  ...
  return (
    <motion.div variants={fadeIn('up', 'spring', index*0.5, 0.75)}>
      <Tilt options={{max:45, scale:1, speed:450}} className='bg-tertiary p-5 rounded-2xl sm:w-[360px] w-full'>
        ...
        <div className='mt-5' onClick={()=>{window.open(deployment_link, 'blank')}} style={{cursor: 'pointer'}}>
