      ...
      <div className="w-full flex justify-between items-center max-w-7xl mx-auto">
        ...
        <Link to={'/'} className="flex items-center gap-2" onClick={()=>{setActive(''); window.scrollTo(0,0);}}>
          ...
          <p className="text-white text-[18px] font-bold cursor-pointer flex">
            Aastik &nbsp;<span className="sm:block hidden">| Developer</span>
          </p>
        </Link>
        ...
        <div className="sm:hidden flex flex-1 justify-end items-center">
          ...
          <div className={`${toggle?'flex':'hidden'} p-6 black-gradient absolute top-20 right-0 mx-4 my-2 min-w-[140px] z-10 rounded-xl`}>
            <ul className="list-none flex justify-end items-start flex-1 flex-col gap-4">
              {navLinks.map((nav)=>(
                <li key={nav.id} className={`font-poppins font-medium cursor-pointer text-[16px] ${active===nav.title?'text-white':'text-secondary'}`} onClick={()=>{setActive(nav.title);setToggle(!toggle);}}>
                  <a href={`#${nav.id}`}>{nav.title}</a>
                </li>
              ))}  
            </ul>            
